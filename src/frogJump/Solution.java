...
public class Solution {
    // over memory limit - this algo only works with small number of possible
    // route.
    public boolean canCross(int[] stones) {
        ...
        Queue<Integer> stoneQueue = new LinkedList<>();
        Queue<Integer> unitQueue = new LinkedList<>();
        ...
        while (!stoneQueue.isEmpty()) {
            ...
            int unit = unitQueue.poll();
            int index = indexQueue.poll();


            for (int i = -1; i <= 1; i++) {
                if (i + unit > 0) {
                    int nextStone = stone + i + unit;
                    if (findStone(nextStone, index, stones, indexQueue)) {
                        if (nextStone == target) {
                            return true;
                        } else {
                            stoneQueue.add(stone + i + unit);
                            unitQueue.add(i + unit);
                        }
                    }
                }
            }
    ...
    private boolean findStone(int stone, int index,int[] stones, Queue<Integer> indexQueue) {
        for(int i= index+1; i< stones.length; i++){
            if (stone == stones[i]) {
                indexQueue.add(i);
                return true;
            } else if (stone < stones[i]) {
                return false;
            }
        }
        return false;

...
public class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        int cnt = 0;
        int maxFillNum = (taskArray[taskArray.length-1]-1)*n;
        int res = taskArray[taskArray.length-1] + maxFillNum;
        int usedForGap = 0;
        for (int i=taskArray.length-2; i>=0; i--) {
            if (taskArray[i]==0) {
                break;
            }
            if (taskArray[i] == taskArray[taskArray.length-1]) {
                usedForGap+=(taskArray[i]-1);
                cnt ++;
                if (cnt<=n){
                    res++;
                }else{
                ...
                if (usedForGap >= maxFillNum) {
                    res = res + (tasks.length-usedForGap - cnt - taskArray[taskArray.length-1]);
                    break;
                }else{
                    if (usedForGap + taskArray[i] > maxFillNum) {
                        res = res + (usedForGap+taskArray[i]-maxFillNum);
                    }
                    usedForGap += taskArray[i];
                }
            }
        }
        return res;
